...
export function buildOpinion({ client, caseRow, data, sources, areaLabel, role }) {
  ...
  ${sectionTitle('א. רקע עובדתי — גרסאות הצדדים')}
  ...
  ${data.strategy ? `${sectionTitle('ה. אסטרטגיה ראשונית')}${nl2p(data.strategy)}` : ''}

  ${sectionTitle(`${data.strategy ? 'ו' : 'ה'}. המלצות לדרך פעולה`)}
